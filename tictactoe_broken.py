...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player # change column -1 tp just column
    return game

def check_space_empty(game, row, column):
    print("space not available") #add print statement
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #add :
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column) #add column
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game) # removed comment # at the beginning / and add space
